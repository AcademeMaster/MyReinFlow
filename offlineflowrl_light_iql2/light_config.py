...
class LitMeanFQL(L.LightningModule):
    ...
    def training_step(self, batch: Dict[str, Tensor], batch_idx: int) -> Dict[str, Any]:
        ...
        observations = observations.squeeze(1)  # 从 [B, 1, obs_dim] 转换为 [B, obs_dim]
        ...
        next_observations = next_observations.squeeze(1)  # 从 [B, 1, obs_dim] 转换为 [B, obs_dim]
        rewards = batch["rewards"].float().to(device)  # [B, H]
        terminations = batch["terminations"].float().to(device)  # [B, 1]

        # 获取优化器
        critic_optimizer, value_optimizer, policy_optimizer = self.optimizers()
        # 更新 step_idx
        self.step_idx += 1
        ...
        if (int(self.step_idx) % self.cfg.q_update_period) == 0:
            ...
            critic_loss, critic_info = self.net.loss_qf(observations, action_chunks, next_observations, rewards, terminations)
        ...
        if (int(self.step_idx) % self.cfg.v_update_period) == 0:
            # Value function更新步骤
            self.toggle_optimizer(value_optimizer)
            value_loss, value_info = self.net.loss_vf(observations, action_chunks)
            value_optimizer.zero_grad()
            self.manual_backward(value_loss)
            if self.cfg.grad_clip_value:
                self.clip_gradients(value_optimizer, gradient_clip_val=self.cfg.grad_clip_value, gradient_clip_algorithm="norm")
            value_optimizer.step()
            self.untoggle_optimizer(value_optimizer)
        ...
        # 计算总损失
        total_loss = critic_loss + value_loss + policy_loss

        # 记录训练日志
        self.log("critic/loss", critic_loss, on_step=True, prog_bar=True)
        self.log("vf/loss", value_loss, on_step=True, prog_bar=True)
        ...
        # 记录详细信息
        info = {**critic_info, **value_info, **policy_info}
    ...
    def validation_step(self, batch: Dict[str, Tensor], batch_idx: int) -> Tensor:
        ...
        observations = observations.squeeze(1) if observations.dim() > 2 else observations
        action_chunks = batch["action_chunks"].float().to(device)
        next_observations = batch["next_observations"].float().to(device)
        next_observations = next_observations.squeeze(1) if next_observations.dim() > 2 else next_observations
        rewards = batch["rewards"].float().to(device)
        rewards = rewards.unsqueeze(-1) if rewards.dim() == 2 else rewards
        terminations = batch["terminations"].float().to(device)
        ...
        # 计算总损失
        validation_loss = critic_loss + value_loss + policy_loss
    ...
    def configure_optimizers(self) -> Tuple[Dict, ...]:
        ...
        value_optimizer = optim.Adam(self.net.vf.parameters(), lr=self.cfg.learning_rate)
        ...
        value_scheduler = optim.lr_scheduler.StepLR(value_optimizer, step_size=10, gamma=0.1)
        ...
        return (
            ...
            {"optimizer": value_optimizer, "lr_scheduler": {"scheduler": value_scheduler, "interval": "epoch"}},
