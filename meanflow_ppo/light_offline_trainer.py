...
class LitMeanFQL(L.LightningModule):
    ...
    def training_step(self, batch: Dict[str, Tensor], batch_idx: int) -> Dict[str, float]:
        """
        训练步骤，根据配置的更新频率更新网络组件
        ...
        if (int(self.step_idx) % self.cfg.policy_update_period) == 0:
            ...
            policy_loss, policy_info = self.net.loss_policy(observations, action_chunks, int(self.step_idx))
    ...
    def validation_step(self, batch: Dict[str, Tensor], batch_idx: int) -> Tensor:
        ...
        # 使用Best-of-N采样方法预测动作
        with torch.no_grad():
            predicted_actions = self.net.best_of_n_sampling(observations, self.cfg.N)

        # 计算动作预测的均方误差（类似在线评估中的策略性能）
        action_mse = F.mse_loss(predicted_actions, action_chunks)

        # 计算回报（每个episode的累积奖励）
        # 奖励序列形状为 [B, H, 1]，我们需要计算每个episode的总奖励
        episode_rewards = torch.sum(rewards, dim=1).squeeze(-1)  # [B]

        # 计算总损失
        validation_loss = critic_loss + policy_loss

        # 记录奖励和详细信息
        self.log("val/reward", episode_rewards.mean(), on_step=False, on_epoch=True, prog_bar=True)
        self.log("val/loss", validation_loss, on_step=False, on_epoch=True, prog_bar=True)
        self.log("val/action_mse", action_mse, on_step=False, on_epoch=True, prog_bar=False)
