...
if __name__ == "__main__":
    ...
    # 保存归一化参数以便后续使用
    if args.normalize:
        np.savez(f"models/{file_name}_online_norm", mean=mean, std=std)

    # 开始在线训练
    print("开始在线训练...")
    print(f"缓冲区大小: {replay_buffer.size}")
    final_replay_buffer = online_train_policy(
        policy, env, replay_buffer, mean, std,
    ...
    # 保存最终模型
    if args.save_model:
        policy.save(f"./models/{file_name}_online_final")
        print(f"最终模型已保存到 ./models/{file_name}_online_final")