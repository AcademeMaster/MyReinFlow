...
class MeanFQL(nn.Module):
    ...
    def loss_vf(self, obs: Tensor, actions: Tensor) -> Tuple[Tensor, Dict]:
        with torch.no_grad():
            ...
            q_pred = torch.min(q1_pred, q2_pred).squeeze(-1)

        vf_pred = self.vf(obs).squeeze(-1)
        vf_err = vf_pred - q_pred
        vf_sign = (vf_err > 0).float()
        vf_weight = (1 - vf_sign) * self.cfg.quantile + vf_sign * (1 - self.cfg.quantile)
...
class LitMeanFQL(L.LightningModule):
    def __init__(self, obs_dim: int, action_dim: int, cfg: Config):
        ...
        # 将计数器移到 LightningModule 中
        self._n_train_steps_total = 0

        self.critic_optimizer = optim.Adam(self.net.critic.parameters(), lr=cfg.learning_rate)
        self.vf_optimizer = optim.Adam(self.net.vf.parameters(), lr=cfg.learning_rate)
        self.policy_optimizer = optim.Adam(self.net.actor.parameters(), lr=cfg.learning_rate)
    ...
    def training_step(self, batch: Dict[str, Tensor], batch_idx: int):
        ...
        # if self._n_train_steps_total % self.cfg.q_update_period == 0:
        #     self.critic_optimizer.zero_grad()
        #     self.manual_backward(qf_loss)
        #     self.critic_optimizer.step()
        #
        # if self._n_train_steps_total % self.cfg.v_update_period == 0:
        #     self.vf_optimizer.zero_grad()
        #     self.manual_backward(vf_loss)
        #     self.vf_optimizer.step()
        #
        # if self._n_train_steps_total % self.cfg.policy_update_period == 0:
        ...
        for k, v in {**qf_info, **vf_info, **policy_info}.items():
