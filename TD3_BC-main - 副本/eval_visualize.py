...
def detect_model_horizon(model_path, state_dim, action_dim):
    """
    检测保存的模型使用的horizon参数
    通过比较Critic网络的输入维度来推断
    """
    try:
        # 尝试加载critic模型来检测维度
        critic_state = torch.load(model_path + "_critic", map_location='cpu')
        # 获取第一层权重的形状
        first_layer_weight = critic_state['Q1_net.0.weight']
        input_dim = first_layer_weight.shape[1]

        # 计算horizon: input_dim = state_dim + action_dim * horizon
        action_part = input_dim - state_dim
        horizon = action_part // action_dim

        if horizon * action_dim + state_dim == input_dim and horizon >= 1:
            return horizon
        else:
            print(f"警告: 无法从模型维度推断horizon，使用默认值1")
            return 1
    except Exception as e:
        print(f"检测horizon时出错: {e}")
        return 1
...
def main():
    ...
    parser.add_argument("--horizon", default=2, type=int, help="手动指定horizon，0为自动检测")
    ...
    if args.horizon > 0:
        ...
        print(f"使用手动指定的horizon: {horizon}")
    elif model_path:
        horizon = detect_model_horizon(model_path, state_dim, action_dim)
        print(f"自动检测的horizon: {horizon}")
    else:
        horizon = 1
        print(f"使用默认horizon: {horizon}")
    ...
    if model_path:
        try:
            ...
            print(f"模型配置 - Actor输出维度: {policy.actor.output_dim}, Critic输入维度: {action_dim * horizon}")
        except Exception as e:
            print(f"加载模型失败: {e}")
            print("使用随机策略进行评估")
