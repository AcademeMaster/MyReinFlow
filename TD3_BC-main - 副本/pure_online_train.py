...
def eval_policy(policy, env_name, seed, mean, std, seed_offset=100, eval_episodes=10):
    ...
    for _ in range(eval_episodes):
        ...
        while not done:
            ...
            if isinstance(state, tuple):
                state_obs = state[0] if len(state) > 0 else state
            else:
                state_obs = state
    ...
    avg_episode_length = total_timesteps / eval_episodes

    print(f"  Average reward: {avg_reward:.3f} | Average length: {avg_episode_length:.1f}")
    return avg_reward


def pure_online_train_policy(policy, env, mean, std, max_timesteps, 
                            eval_freq, expl_noise, start_timesteps=10000, batch_size=256):
    """
    纯在线训练策略，不使用任何离线数据，仅通过与环境交互学习
    """
    # 设置为在线训练模式，不使用BC损失
    ...
    replay_buffer = utils.ReplayBuffer(state_dim, action_dim, horizon=policy.horizon)
    ...
    for t in range(int(max_timesteps)):
        ...
        replay_buffer.add(state_normalized.flatten(), action, next_state_normalized.flatten(), 
                         reward, float(done), episode_start=(episode_timesteps == 1))
        ...
        if done:
            # 重置环境和action cache
            policy.reset_action_cache()  # 重要：episode结束时重置action cache
            ...
            if (episode_num + 1) % max(1, int(eval_freq / 1000)) == 0:
                ...
                np.save(f"models/{file_name}_pure_online_eval", evaluations)
                
                # 保存模型
                policy.save(f"./models/{file_name}_pure_online")
...
if __name__ == "__main__":
    ...
    # Load Minari dataset and recover environment (仅用于获取环境信息)
    ...
    if args.normalize:
        ...
        for _ in range(1000):
            ...
            if isinstance(state, tuple):
                ...
                state_obs = state
            state_array = np.asarray(state_obs, dtype=np.float32)
            sample_states.append(state_array)
        
        states_array = np.array(sample_states)
        mean = np.mean(states_array, axis=0)
        std = np.std(states_array, axis=0) + 1e-3  # 添加小量避免除零
    else:
        mean, std = 0, 1

    # 保存归一化参数以便后续使用
    if args.normalize:
        np.savez(f"models/{file_name}_pure_online_norm", mean=mean, std=std)
    ...
    # 开始纯在线训练
    print("开始纯在线训练...")
    final_replay_buffer = pure_online_train_policy(
        policy, env, mean, std,
    ...
    # 保存最终模型
    if args.save_model:
        policy.save(f"./models/{file_name}_pure_online_final")
        print(f"最终模型已保存到 ./models/{file_name}_pure_online_final")