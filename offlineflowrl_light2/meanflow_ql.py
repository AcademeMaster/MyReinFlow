...
class MeanFlowActor(nn.Module):
    ...
    def predict_action_chunk(self, obs: Tensor, n_steps: int = 1) -> Tensor:
        ...
        with torch.no_grad():
            # 直接使用sample_mean_flow的结果，不再需要额外的tanh缩放
...
class MeanFQL(nn.Module):
    ...
    def best_of_n_sampling(self, obs: Tensor, N: int) -> Tensor:
        ...
        返回:
            [B, pred_horizon, action_dim] 最优动作序列
        """
        with torch.no_grad():
            # 并行化采样N个候选动作序列
    ...
    def loss_actor(self, obs: Tensor, action_batch: Tensor) -> Tuple[Tensor, Dict]:
        ...
        # 组合损失函数
        loss = q_weight * q_loss * 0 + bc_weight * bc_loss
        info = dict(
    ...
    def total_loss(self, obs: Tensor, actions: Tensor, next_obs: Tensor,
                   rewards: Tensor, terminated: Tensor, gamma: float) -> Tuple[Tensor, Dict]:
        """
        统一的损失函数
        """
        loss_c, info_c = self.loss_critic(obs, actions, next_obs, rewards, terminated, gamma)
        loss_a, info_a = self.loss_actor(obs, actions)
        total_loss = loss_c + loss_a
        info = {**info_c, **info_a}
...
class LitMeanFQL(L.LightningModule):
    ...
    def validation_step(self, batch: Dict[str, Tensor], batch_idx: int):
        ...
        # 使用统一的损失函数进行验证
        val_loss, info = self.net.total_loss(obs, actions, next_obs, rewards, terminated, self.cfg.gamma)
