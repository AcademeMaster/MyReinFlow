...
class ReplayBuffer:
    ...
    def _get_multi_step_reward(self, start_idx: int, gamma: float = 0.99) -> float:
        """
        Calculate multi-step discounted reward.
        正确处理episode边界，确保不跨越episode
        ...
        Returns:
            Multi-step discounted reward
        """
        current_size = min(self.size, self.max_size)
        total_reward = 0.0
        discount = 1.0  # 修复：初始折扣应该是1.0
        ...
        for i in range(self.horizon):
            ...
            if i > 0:
                prev_idx = (start_idx + i - 1) % current_size
                # 如果前一步是episode结束，停止累积
                if self.episode_ends[prev_idx]:
                    break
                # 如果当前步是episode开始，说明跨越了边界，停止累积
                if self.episode_starts[idx]:
                    break

            total_reward += discount * self.reward[idx, 0]
            discount *= gamma  # 正确使用传入的gamma参数

        return total_reward

    def _is_valid_chunk_start(self, start_idx: int) -> bool:
        """
        检查从start_idx开始是否能提供有效的action chunk和reward horizon

        Args:
            start_idx: Starting index to check

        Returns:
            Boolean indicating if this is a valid start position
        """
        current_size = min(self.size, self.max_size)
        ...
        for i in range(1, self.horizon):
            ...
            # 如果遇到episode边界，检查是否还有足够的数据
            if self.episode_ends[prev_idx] or self.episode_starts[idx]:
                # 对于action chunking，至少需要一个有效action
                # 对于reward horizon，至少需要一个有效reward
                return True

        return True

    def sample(self, batch_size: int, return_chunks: bool = False, gamma: float = 0.99):
        """
        ...
        Args:
            ...
            return_chunks: Whether to return action chunks and multi-step rewards
            gamma: Discount factor for multi-step rewards

        Returns:
            If return_chunks=False: Tuple of torch tensors (states, actions, next_states, rewards, dones)
            If return_chunks=True: Tuple of torch tensors (states, action_chunks, next_states, multi_step_rewards, dones)
        """
        current_size = min(self.size, self.max_size)
        if current_size == 0:
        ...
        if not return_chunks:
            ...
            # Chunk采样，需要考虑边界约束
            # 为了安全起见，限制采样范围
            if current_size < self.horizon:
                # 如果buffer大小小于horizon，仍然可以采样，但效果可能不好
                valid_start_positions = list(range(current_size))
            else:
                # 正常情况下，确保有足够的后续数据
                valid_start_positions = list(range(current_size))

            if len(valid_start_positions) == 0:
                raise ValueError("No valid start positions for chunk sampling")

            if batch_size > len(valid_start_positions):
                # 允许重复采样
                indices = np.random.choice(valid_start_positions, size=batch_size, replace=True)
            else:
                indices = np.random.choice(valid_start_positions, size=batch_size, replace=False)

            # 获取action chunks和multi-step rewards
            action_chunks = []
            multi_step_rewards = []
            ...
            for idx in indices:
                ...
                # 获取multi-step reward
                multi_step_reward = self._get_multi_step_reward(idx, gamma)
                multi_step_rewards.append(multi_step_reward)

            action_chunks = np.stack(action_chunks)  # (batch_size, chunk_size, action_dim)
            multi_step_rewards = np.array(multi_step_rewards).reshape(-1, 1)  # (batch_size, 1)

            return (
                torch.FloatTensor(self.state[indices]).to(self.device),
                torch.FloatTensor(action_chunks).to(self.device),
                torch.FloatTensor(self.next_state[indices]).to(self.device),
                torch.FloatTensor(multi_step_rewards).to(self.device),
                torch.FloatTensor(self.done[indices]).to(self.device)
            )
    ...
    def convert_D4RL(self, dataset: dict):
        ...
        # 设置episode边界
        self.episode_starts[0] = True  # 第一个样本是episode开始
