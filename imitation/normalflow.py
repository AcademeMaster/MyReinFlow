...
class FlowPolicyAgent:
    ...
    @torch.no_grad()
    def predict_action_chunk(self, batch: Dict[str, torch.Tensor],n_steps=10) -> torch.Tensor:
        """
        预测动作块
        ...
        for step in range(n_steps):
            t_val = torch.full((observations.size(0),), step*dt, device=device, dtype=torch.float32)
            update = self.model(obs_cond, t_val, noise)
            noise = noise + dt * update
