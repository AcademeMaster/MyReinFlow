...
def test_data_arrangement():
    ...
    if sample_size > 0:
        states, actions, rewards, next_states, dones, actual_steps, early_termination = buffer.sample(sample_size)
        ...
        for i in range(sample_size):
            ...
            print(f"  actual_steps: {actual_steps[i].item()}")
            print(f"  early_termination: {early_termination[i].item()}")
    ...
    if sample_size > 0:
        ...
        for i in range(sample_size):
            expected_length = actual_steps[i].item()
            action_len = len(actions[i])
            reward_len = len(rewards[i])
            # dones现在是单个值，不需要检查长度
            consistent = (action_len == reward_len == expected_length)
            
            if not consistent:
                print(f"  ✗ 样本{i}: 长度不一致 - actions:{action_len}, rewards:{reward_len}, expected:{expected_length}")
                all_consistent = False
            else:
                print(f"  ✓ 样本{i}: 长度一致 ({expected_length})")
...
def test_multiple_episodes():
    ...
    if len(buffer) >= 5:
        print("\n采样验证 (5个样本):")
        states, actions, rewards, next_states, dones, actual_steps, early_termination = buffer.sample(5)
