...
class TD3:
    ...
    def compute_discounted_reward(self, rewards_list, actual_steps):
        """计算多步折扣奖励（并行优化版本）
        ...
        max_steps = max(len(rewards) for rewards in rewards_list)
        
        # 创建填充的奖励矩阵 [batch_size, max_steps]
        rewards_matrix = torch.zeros(batch_size, max_steps).to(self.device)
        mask = torch.zeros(batch_size, max_steps, dtype=torch.bool).to(self.device)
        
        # 并行填充奖励和掩码
        for i, rewards in enumerate(rewards_list):
            steps = len(rewards)
            rewards_matrix[i, :steps] = torch.tensor(rewards, dtype=torch.float).to(self.device)
            mask[i, :steps] = True
        
        # 创建gamma矩阵 [1, max_steps]
        if max_steps > len(self.gamma_powers):
            gamma_matrix = torch.pow(self.discount, torch.arange(max_steps, dtype=torch.float).to(self.device)).unsqueeze(0)
        else:
            gamma_matrix = self.gamma_powers[:max_steps].unsqueeze(0)
        
        # 并行计算折扣奖励：逐元素相乘后按行求和
        discounted_matrix = rewards_matrix * gamma_matrix * mask.float()
    ...
    def update(self, batch_size):
        ...
        # 检查是否使用MultiStepReplayBuffer
        if hasattr(self.replay_buffer, 'multi_step_data'):
            # 多步采样
            states, actions_list, rewards_list, next_states, dones_list, actual_steps, early_termination = self.replay_buffer.sample(batch_size)
            
            # 处理多步动作：将动作序列转换为action chunk格式
            actions = torch.zeros(batch_size, self.action_chunk_dim).to(self.device)
            for i, action_seq in enumerate(actions_list):
                # 将动作序列展平为action chunk
                flat_actions = []
                for j, action in enumerate(action_seq):
                    if j < self.action_horizon:  # 只取前action_horizon个动作
                        if isinstance(action, (list, np.ndarray)):
                            flat_actions.extend(action)
                        else:
                            flat_actions.append(action.item() if hasattr(action, 'item') else action)
                # 如果动作不足，用最后一个动作填充
                if len(action_seq) > 0:
                    last_action = action_seq[-1]
                    if isinstance(last_action, (list, np.ndarray)):
                        last_action_list = list(last_action)
                    else:
                        last_action_list = [last_action.item() if hasattr(last_action, 'item') else last_action]
                    while len(flat_actions) < self.action_chunk_dim:
                        flat_actions.extend(last_action_list)
                actions[i] = torch.tensor(flat_actions[:self.action_chunk_dim], dtype=torch.float).to(self.device)
            
            # 处理dones
            dones = torch.tensor(dones_list, dtype=torch.float).view(-1, 1).to(self.device)
            
        else:
            # 单步采样（兼容原有ReplayBuffer）
            states, actions, rewards, next_states, dones = self.replay_buffer.sample(batch_size)
            # 将单步数据转换为多步格式
            rewards_list = [[r.item()] for r in rewards]
            actual_steps = torch.ones(batch_size, dtype=torch.long).to(self.device)
        ...
        with torch.no_grad():
            ...
            # 计算多步折扣奖励
            discounted_rewards = self.compute_discounted_reward(rewards_list, actual_steps)
            
            # 计算目标Q值：G + γ^n * Q(s', a')
            target_Q = discounted_rewards + (1 - dones) * (self.discount ** actual_steps.float().view(-1, 1)) * target_Q
