...
class TD3:
    ...
    def compute_discounted_reward(self, rewards_list, actual_steps):
        ...
        """
        if isinstance(rewards_list, torch.Tensor):
            rewards_tensor = rewards_list.detach().clone().to(self.device)
        else:
    ...
    def update(self, batch_size):
        ...
        with torch.no_grad():
            ...
            # 计算多步折扣奖励
            discounted_rewards = self.compute_discounted_reward(rewards, self.action_horizon)
            
            # 计算目标Q值：G + γ^n * Q(s', a')
            # dones是bool类型，需要转换为float
   
            target_Q = discounted_rewards + (1 - dones) * (self.discount ** self.action_horizon) * target_Q

        # Get current Q estimates
        # 将actions从[batch_size, action_horizon]扩展为[batch_size, action_horizon*action_dim]
        # 这里actions是[batch_size, action_horizon]，但critic期望[batch_size, action_chunk_dim]
        # 我们需要将每个action复制action_dim次来匹配期望的维度
        batch_size, action_horizon = actions.shape
        actions_expanded = actions.unsqueeze(-1).expand(batch_size, action_horizon, self.action_dim)
        actions_flat = actions_expanded.contiguous().view(batch_size, -1)  # [batch_size, action_horizon*action_dim]
        
        current_Q1, current_Q2 = self.critic(states, actions_flat)
