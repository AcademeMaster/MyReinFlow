...
class MultiStepReplayBuffer:
    ...
    def _process_episode(self):
        ...
        for start_idx in range(new_episode_lin):
            ...
            sample = {
                'state': [episode_slice[-1]['state']], 
                'actions': [step['action'] for step in episode_slice],
                'rewards': [step['reward'] for step in episode_slice],
                'next_state': [episode_slice[-1]['next_state']],  # 只有最后一个done值
                'dones': [episode_slice[-1]['done']],  # 只有最后一个done值
            }

            # states: torch.Size([256, 1, 3])
            # actions: torch.Size([256, 4, 1])
            # rewards: torch.Size([256, 4])
            # next_states: torch.Size([256, 1, 3])
            # dones: torch.Size([256, 1])# 转为一维
