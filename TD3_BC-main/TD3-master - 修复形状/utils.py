...
class MultiStepReplayBuffer:
    ...
    def _process_episode(self):
        ...
        for i in range(new_episode_lin):
            ...
            self.multi_step_data.append({
                'state': [self.current_episode[i]['state']],  # state[i]
                'actions': [self.current_episode[i+j]['action'] for j in range(self.action_horizon)],  # action[i:i+h]
                'rewards': [self.current_episode[i+j]['reward'] for j in range(self.action_horizon)],  # reward[i:i+h]
                'next_state': [self.current_episode[i+self.action_horizon-1]['next_state']],  # next_state[i+h-1]
                'dones': [self.current_episode[i+self.action_horizon-1]['done']],  # done[i+h-1]
            })
    ...
    def sample(self, batch_size):
        ...
        # 转换为tensor并调整形状
        states = torch.from_numpy(np.array(states)).float().to(self.device).flatten(1)  # [256, 1, 23] -> [256, 23]
        next_states = torch.from_numpy(np.array(next_states)).float().to(self.device).flatten(1)  # [256, 1, 23] -> [256, 23]
        rewards = torch.from_numpy(np.array(rewards)).float().to(self.device).flatten(1)  # [256, 4] 保持不变
        dones = torch.from_numpy(np.array(dones)).float().to(self.device).flatten(1)      # [256, 1] -> 保持 
        actions = torch.from_numpy(np.array(actions_np)).float().to(self.device).flatten(1)  # [256, 4, 7] -> [256, 28]
