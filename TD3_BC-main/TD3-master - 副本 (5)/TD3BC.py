...
class TD3:
    ...
    noise_clip=0.5, policy_freq=2, replay_buffer=None, action_horizon=1,train_mode="online"):
        ...
        self.actor =  MeanFlowPolicy(obs_dim=state_dim, action_dim=self.action_chunk_dim, max_action=self.max_action, device=device)
    ...
    def take_action(self, state, add_noise=True):
        ...
        if len(self.action_deque) == 0:
            ...
            action_chunk = action_chunk.view(self.action_horizon, self.action_dim)
            actions_list = [action_chunk[i].clone() for i in range(self.action_horizon)]
            self.action_deque.extend(actions_list)
        # 取出第一个动作
    ...
    def compute_discounted_reward(self, rewards_tensor, actual_steps):
        """计算多步折扣奖励（并行优化版本）
        
        Args:
            rewards_tensor: torch.Tensor - 每个样本的奖励序列
        ...
        gamma_matrix = torch.pow(self.discount, torch.arange(self.action_horizon, dtype=torch.float).to(self.device)).unsqueeze(0)        
        # 并行计算折扣奖励：逐元素相乘后按行求和
        discounted_matrix = rewards_tensor * gamma_matrix
    ...
    def update(self, batch_size):
        ...
        # 多步采样
        states, actions, rewards_tensor, next_states, dones, actual_steps, early_termination = self.replay_buffer.sample(batch_size)
        with torch.no_grad():
            ...
            # 计算多步折扣奖励
            discounted_rewards = self.compute_discounted_reward(rewards_tensor, self.action_horizon)
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            # Compute actor loss
            actor_loss = 0
            if self.train_mode=="online":
                actor_loss = -self.critic.Q1Value(states, self.actor(states)).mean()
            elif self.train_mode=="offline":
                # Q loss + BC loss
                actor_loss = -self.critic.Q1Value(states, self.actor(states)).mean()+self.actor.per_sample_flow_bc_loss(states, actions)
